...
const ReportProofOfPayment = () => {
  ...
  const generatePDFMemo = useMemo(() => {
    ...
    if (data?.length) {
      getReportCompetitionAwardState.result?.data?.rows?.map((row) => {
        ...
        body.push(['#', 'ระดับกิจกรรม', 'รายการเข้าร่วม', 'รายชื่อผู้สมัคร', 'สถานศึกษา', 'จำนวนเงิน']);
        row.programs?.map((row, index) => {
          if (index == 0 || row.competitions?.length == 1) {
            body.push([
              ...
              { text: '1' },
              {
                text: row.competitions[0]?.participants?.map((row) => row.participant_name + '\n').join('') ?? '',
              },
              { text: row.competitions[0]?.member?.school?.name ?? '' },
              { text: formatter.thb(row.competitions[0]?.competition_result?.amount ?? 0) },
            ]);
          } else {
            body.push([
              { text: `${index + 1}` },
          ...
          for (let index = 1; index < row.competitions?.length; index++) {
            body.push([
              ...
              `${formatter.thb(row.competitions[index]?.competition_result?.amount ?? '')}`,
            ]);
          }
        });
      });
  ...
  const docDefinition = {
    ...
    content: [
      { text: 'รายงานไฟล์หลักฐานการจ่ายเงิน', style: 'header' },
      { text: '\n' },
      {
        ...
        table: {
          ...
          body: [
            ...
            ['จำนวนเงินทั้งหมด', `${formatter.thb(getReportCompetitionAwardState.result?.data?.sum_competition_payment ?? '')}`],
          ],
        },
      },
      { text: '\n' },
        ...
        <Box marginBottom={3}>
          <Stack mt={1} mb={1}>
            <Stack mb={1} direction="row" justifyContent="space-between">
              <Typography variant="h6">รายงานไฟล์หลักฐานการจ่ายเงิน</Typography>
              <Button variant="outlined" onClick={downloadPDF} disabled={!getReportCompetitionAwardState.result?.data}>
                ดาวน์โหลด PDF
              </Button>
            </Stack>
          </Stack>
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow sx={{ verticalAlign: 'top' }}>
                      ...
                      <TableCell>{getReportCompetitionAwardState.result?.data?.count_team ?? ''}</TableCell>
                    </TableRow>
                    <TableRow sx={{ verticalAlign: 'top' }}>
                      <TableCell sx={{ fontWeight: 'bold' }}>จำนวนทีมที่ได้รับรางวัล</TableCell>
                      <TableCell>
                        {getReportCompetitionAwardState.result?.data?.count_competition_award ?? ''}
                      </TableCell>
                    </TableRow>
                    <TableRow sx={{ verticalAlign: 'top' }}>
                      <TableCell sx={{ fontWeight: 'bold' }}>จำนวนเงินทั้งหมด</TableCell>
                      <TableCell>
                        {formatter.thb(getReportCompetitionAwardState.result?.data?.sum_competition_payment ?? '')}
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
                      ...
                      <TableBody>
                        {row.programs?.map((row, index) => (
                          <Fragment key={row.program_id}>
                            <TableRow sx={{ verticalAlign: 'top' }}>
                              <TableCell rowSpan={(row.competitions.length + 1).toString()}>{index + 1}</TableCell>
...
export default ReportProofOfPayment;
