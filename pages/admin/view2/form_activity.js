...
const FormActivity = () => {
  ...
  useEffect(() => {
    ...
    if (activityId) {
      dispatch(actions.getFormActivityByIdRequest(activityId));
    }
  ...
  const addFormActivityState = useSelector(({ addFormActivityReducer }) => addFormActivityReducer);
  const getFormActivityByIdState = useSelector(
    ({ getFormActivityByIdReducer }) => getFormActivityByIdReducer
  );
  const updateFormActivityState = useSelector(
    ({ updateFormActivityReducer }) => updateFormActivityReducer
  );

  // set form activity max no
  const formActivityMaxNo = useMemo(() => {
    const formActivitiesLength =
      getFormActivityByIdState?.result?.data?.form_activities?.length - 1;
    const maxNo =
      getFormActivityByIdState?.result?.data?.form_activities[formActivitiesLength]?.no + 1;
    return maxNo ? maxNo : 1;
  }, [getFormActivityByIdState]);
  ...
  const [initialFormActivity, setInitialFormActivity] = useState({
    activity_id: '',
    form_activity_id: '',
    no: '',
    type: '',
  ...
  const addFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      dispatch(actions.addFormActivityRequest(data));
    },
  });
  ...
  const editFormik = useFormik({
    enableReinitialize: true,
    initialValues: {
      activity_id: initialFormActivity.activity_id,
      no: initialFormActivity.no,
      type: initialFormActivity.type,
      name: initialFormActivity.name,
      required: initialFormActivity.required,
    },
    validate: (values) => {
      const errors = {};
      if (!values.no) {
    ...
    onSubmit: async (values) => {
        ...
        for await (const [i] of radioInputRef.current.entries()) {
          ...
          if (radioInputRef.current[i].name === 'create') {
            ...
            updateeRadioList.push({
              form_activity_radio_id: radioInputRef.current[i].id,
      ...
      data = {
        id: initialFormActivity.form_activity_id,
        no: values.no,
        type: values.type,
        name: values.name,
        required: values.required,
      ...
      if (initialFormActivity.type == 2 && (values.type == 1 || values.type == 3)) {
        // save id to delete
        var arr = [];
        for await (const [i] of radioInput.entries()) {
          arr.push({ form_activity_radio_id: radioInput[i].form_activity_radio_id });
        }
        data.delete_radio_list = arr;
      }
      dispatch(actions.updateFormActivityRequest(data));
    },
  });
  // END EDIT SECTION //

  // handle radio options
  const [radioInput, setRadioInput] = useState([{ form_activity_radio_id: uuidv4(), name: '' }]);
  const [deleteRadioList, setDeleteRadioList] = useState([]);
  const radioInputRef = useRef([]);

  const handleRadioInput = (action, form_activity_radio_id, createdAt, updatedAt) => {
    if (action === 'add') {
      setRadioInput([...radioInput, { form_activity_radio_id: uuidv4() }]);
      ...
      if (radioInput.length > 1) {
        confirm({ description: 'ต้องการยืนยันรายชื่ออาจารย์ที่ปรึกษาหรือไม่?' }).then(() => {
          ...
          if (createdAt && updatedAt) {
            setDeleteRadioList([
              ...deleteRadioList,
              { form_activity_radio_id: form_activity_radio_id },
            ]);
          }
          // filter and remove
          let filter = radioInput.filter((row) => {
            return row.form_activity_radio_id != form_activity_radio_id;
          });
        ...
        <Box marginBottom={3}>
          <Stack mt={1} mb={1}>
            <Stack mb={1} direction="row" justifyContent="space-between">
              <Typography variant="h6">จัดการแบบสอบถามกิจกรรม</Typography>
            </Stack>
          </Stack>
          <TabContext value={tabPanel}>
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
              <TabList onChange={handleChangeTab} aria-label="tabs">
                ...
                <Tab label="คำถาม" value="2" disabled={!getFormActivityByIdState?.result} />
              </TabList>
            </Box>
            <TabPanel value="1">
                    ...
                    <List key={index}>
                      <ListItem
                        onClick={() => {
                          ...
                          dispatch(actions.getFormActivityByIdRequest(row.activity_id));
                          setTabPanel('2');
                        }}
                        disablePadding
            ...
            <TabPanel value="2">
              {getFormActivityByIdState.isFetching ? (
                <Loading />
              ) : (
                <Fragment>
                  <Button
                  ...
                  >
                    เพิ่มแบบสอบถามกิจกรรม
                  </Button>
                  <Typography mt={2} mb={1} variant="h6">
                    {getFormActivityByIdState?.result?.data?.activity_name ?? ''}
                  </Typography>
                  <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 650 }} aria-label="simple table">
                      ...
                      <TableBody>
                        {getFormActivityByIdState?.result?.data?.form_activities?.map(
                          (row, index) => (
                            <Fragment key={index}>
                              <TableRow sx={{ verticalAlign: 'top' }}>
                                ...
                                <TableCell>
                                  {row.type == 1 ? (
                                    ...
                                    <>
                                      ...
                                      {row?.form_activity_radioes?.map((row, index) => (
                                ...
                                <TableCell>
                                  <IconButton
                                    ...
                                    component="span"
                                    onClick={() => {
                                      setInitialFormActivity({
                                        activity_id: row.activity_id,
                                        form_activity_id: row.form_activity_id,
                                        no: row.no,
                                        type: row.type,
                                        name: row.name,
                                        required: Boolean(row.required),
                                      });
                                      if (row?.form_activity_radioes?.length) {
                                        setRadioInput(row.form_activity_radioes);
                                      }
                                ...
                                <TableCell>
                                  <IconButton
                                    ...
                                    onClick={() => {
                                      confirm({
                                        description: 'ต้องการยืนยันการลบแบบสอบถามกิจกรรมหรือไม่?',
                                      }).then(() => {
                                        dispatch(
                                          actions.deleteFormActivityRequest(row.form_activity_id)
                                        );
                                      });
                                    }}
                                  >
                  ...
                  <form onSubmit={addFormik.handleSubmit}>
                    ...
                    {addFormik.values.type == 2 && (
                      <>
                        ...
                        {radioInput?.map((row, index) => (
                          <Stack
                            key={row.form_activity_radio_id}
                            flexDirection="row"
                            alignItems="center"
                          >
                            <TextField
                            ...
                            <IconButton
                              sx={{ ml: 1 }}
                              onClick={() => handleRadioInput('remove', row.form_activity_radio_id)}
                            >
                    ...
                    <Stack mt={2} mb={1}>
                      <Button
                        ...
                        color="primary"
                        fullWidth
                        disabled={addFormActivityState.isFetching}
                      >
                        บันทึก
                      </Button>
                    </Stack>
                  ...
                  <form onSubmit={editFormik.handleSubmit}>
                    ...
                    <Stack mt={2} mb={1}>
                      <Button
                        ...
                        color="primary"
                        fullWidth
                        disabled={updateFormActivityState.isFetching}
                      >
                        แก้ไข
                      </Button>
                    </Stack>
