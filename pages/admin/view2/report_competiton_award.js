...
const ReportCompetitonAward = () => {
  ...
  useEffect(() => {
    ...
    dispatch(actions.getReportCompetitionAwardByAdminIdRequest(user?.id));
  }, [router, user]);

  // global state
  const profile = useSelector(({ getAdminByIdReducer }) => getAdminByIdReducer);
  const getReportCompetitionAwardByAdminIdState = useSelector(
    ({ getReportCompetitionAwardByAdminIdReducer }) => getReportCompetitionAwardByAdminIdReducer
  );

  // generate pdf memo
  const generatePDFMemo = useMemo(() => {
    let body = [];
    let data = getReportCompetitionAwardByAdminIdState.result?.data?.rows;
    // map array
    if (data?.length) {
      getReportCompetitionAwardByAdminIdState.result?.data?.rows?.map((row) => {
        body.push([
          {
        ...
        body.push(['#', 'ระดับกิจกรรม', 'ลำดับ', 'รายชื่อผู้สมัคร', 'สถานศึกษา', 'จำนวนเงิน']);
        row.programs?.map((row, index) => {
          ...
          for (let index = 1; index < row.competitions?.length; index++) {
            body.push([
              ...
              `${formatter.thb(row.competitions[index]?.competition_result?.amount ?? 0)}`,
            ]);
          }
        });
      });
    ...
    return body && body;
  }, [router, getReportCompetitionAwardByAdminIdState]);
      ...
      { text: '\n' },
      {
        ...
        table: {
          ...
          body: [
            ...
            [
              'จำนวนทีมทั้งหมด',
              `${getReportCompetitionAwardByAdminIdState.result?.data?.count_team ?? ''}`,
            ],
            [
              'จำนวนทีมที่ได้รับรางวัล',
              `${getReportCompetitionAwardByAdminIdState.result?.data?.colunt_team_with_payment ?? ''}`,
            ],
          ],
        },
      },
          ...
          <Stack mt={1} mb={1}>
            <Stack mb={1} direction="row" justifyContent="space-between">
              ...
              <Button
                variant="outlined"
                onClick={downloadPDF}
                disabled={!getReportCompetitionAwardByAdminIdState.result?.data}
              >
                ดาวน์โหลด PDF
              </Button>
            </Stack>
          </Stack>
          {getReportCompetitionAwardByAdminIdState.isFetching ? (
            <Loading />
          ) : (
            <Fragment>
              ...
              {getReportCompetitionAwardByAdminIdState.result?.data?.rows?.map((row, index) => (
                <Fragment key={index}>
